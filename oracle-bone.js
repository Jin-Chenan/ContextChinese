...
(function injectOracleBoneStyle(){
  ...
  style.textContent =
    ...
    // Each historical script form is tinted while it's showing — light blue for
    // oracle bone, teal for seal script — and the character returns to the
    // page's normal text colour when it crossfades back to the modern glyph.
    // Colour only: the ghost clone keeps the class of the form it's fading OUT
    // of, so the outgoing glyph fades in its own colour rather than snapping.
    // Bone is exactly #90D5FF (the light blue picked for it) in both themes.
    // Seal script's teal is a step deeper on the light theme than the dark
    // one so its thin strokes stay legible against the cream paper.
    ":root{--ob-bone-color:#90D5FF; --ob-seal-color:#149287;}" +
    ":root[data-theme=\"dark\"]{--ob-bone-color:#90D5FF; --ob-seal-color:#4FD1C4;}" +
    ".ob-char-inner.ob-bone, .ob-char-ghost.ob-bone{font-family:'Oracular', var(--serif-cn); color:var(--ob-bone-color);}" +
    ".ob-char-inner.ob-seal, .ob-char-ghost.ob-seal{font-family:'SealScript', var(--serif-cn); color:var(--ob-seal-color);}" +
    ...
    // The dots take on the colour of the script form they're counting (see
    // --ob-bone-color / --ob-seal-color above); outside a stage they stay the
    // normal page accent. Same stage classes that already choose how many show.
    ".ob-char.ob-stage-bone .ob-dot{background:var(--ob-bone-color);}" +
    ".ob-char.ob-stage-seal .ob-dot{background:var(--ob-seal-color);}";
