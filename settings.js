...
const DEFAULT_SETTINGS = {
  ...
  hiddenGeoRegions: [],        // province/region keys excluded from the Geography map & quick-select — see GEO_REGIONS
  geoRegionColors: {},         // e.g. {"sichuan":"#c0392b"} — per-region custom map fill color override, keyed by region key
  ...
  dailyStudyEnabled: true,    // Daily Study: master on/off for the feature itself — when false, the per-collection Daily Study checkbox (the sunrise icon on Word Bank's own row and on each folder pill) is hidden everywhere instead of just being left unchecked, so someone who doesn't use Daily Study doesn't have to look at a control for it. Defaults true so this new setting doesn't hide anything for existing users until they actually turn it off.
  dailyStudyNewPerDay: 10,     // Daily Study: how many brand-new words get introduced per calendar day, shared across every Word Bank/folder collection that has Daily Study turned on (see Word Bank's per-collection toggle)
  mcMatchesBeforeShuffle: 3,   // Multiple Choice: how many of the board's 5 word/definition pairs must be matched before the rest are reshuffled back into the deck and a fresh 5+5 is dealt. 3 (default) shuffles in new words once just over half the board is cleared; 5 waits for the whole board, lower values shuffle in sooner still.
  ...
  tsAudioOnCorrect: false,     // Translate: speak the word aloud once its definition is typed correctly
  forceStopAudioOnNext: false, // Cuts off any still-playing audio the moment the next card is displayed, in Type Pinyin, Match Pinyin, Writing Practice, Translate, and Dictate
  ...
  leftHandedMode: false,       // mirrors every practice page's side-column layout (mode buttons, progression/control buttons, tone key, and Match Pinyin's keycap keyboard) to the opposite side, in both normal and fullscreen view, and carries the same mirroring into each page's idle-preview animation
  parentalControlsEnabled: false, // filters vulgar/explicit/gambling content out of CC-CEDICT definitions (sense-by-sense — a word keeps its other meanings) and swaps in family-friendly text for the few pieces of hand-written site content flagged the same way (e.g. Geography's Macau note)
};
...
/* ---------- left-handed mode ---------- */
// Mirrors every practice page's side-column layout (mode-selector buttons,
// progression/control buttons, a lone tone key, Match Pinyin's keycap
// keyboard) to the opposite side. Each practice page owns its own CSS for
// what "mirrored" means for its particular columns (see that page's own
// is-fullscreen / non-fullscreen grid rules), all gated behind this single
// html[data-left-handed="1"] attribute — same before-first-paint pattern as
// applyTheme() above, so a left-handed user never sees a flash of the
// right-handed layout on load.
function applyLeftHandedMode(){
  if(loadSettings().leftHandedMode) document.documentElement.setAttribute('data-left-handed', '1');
  else document.documentElement.removeAttribute('data-left-handed');
}
applyLeftHandedMode();

/* ---------- nav category visibility ----------
   Each top-level nav-item carries data-nav-cat="learn|vocabulary|hanzi|pinyin|
...
// Geography and History share a single "China +" top-level nav item (see
// applyNavVisibility below) — one combined 'china' entry here, not two, so
// toggling/reordering it in Settings → Navigation acts on the whole merged
// dropdown as a unit rather than exposing sub-parts that no longer have
// their own nav button to hide.
...
// The 34 provincial-level divisions shown on the Geography map — same keys
// geography.html's own mapRegions/provinces arrays use, kept here too since
// Settings needs the list of what to toggle without loading the whole map's
// path data just for a list of labels.
const GEO_REGIONS = [
  {key:'heilongjiang', cn:'黑龙江', en:'Heilongjiang'},
  {key:'jilin', cn:'吉林', en:'Jilin'},
  {key:'liaoning', cn:'辽宁', en:'Liaoning'},
  {key:'neimenggu', cn:'内蒙古', en:'Inner Mongolia'},
  {key:'xinjiang', cn:'新疆', en:'Xinjiang'},
  {key:'xizang', cn:'西藏', en:'Tibet'},
  {key:'qinghai', cn:'青海', en:'Qinghai'},
  {key:'gansu', cn:'甘肃', en:'Gansu'},
  {key:'ningxia', cn:'宁夏', en:'Ningxia'},
  {key:'shaanxi', cn:'陕西', en:'Shaanxi'},
  {key:'shanxi', cn:'山西', en:'Shanxi'},
  {key:'hebei', cn:'河北', en:'Hebei'},
  {key:'beijing', cn:'北京', en:'Beijing'},
  {key:'tianjin', cn:'天津', en:'Tianjin'},
  {key:'shandong', cn:'山东', en:'Shandong'},
  {key:'henan', cn:'河南', en:'Henan'},
  {key:'jiangsu', cn:'江苏', en:'Jiangsu'},
  {key:'anhui', cn:'安徽', en:'Anhui'},
  {key:'shanghai', cn:'上海', en:'Shanghai'},
  {key:'hubei', cn:'湖北', en:'Hubei'},
  {key:'zhejiang', cn:'浙江', en:'Zhejiang'},
  {key:'hunan', cn:'湖南', en:'Hunan'},
  {key:'jiangxi', cn:'江西', en:'Jiangxi'},
  {key:'fujian', cn:'福建', en:'Fujian'},
  {key:'guizhou', cn:'贵州', en:'Guizhou'},
  {key:'chongqing', cn:'重庆', en:'Chongqing'},
  {key:'sichuan', cn:'四川', en:'Sichuan'},
  {key:'yunnan', cn:'云南', en:'Yunnan'},
  {key:'guangxi', cn:'广西', en:'Guangxi'},
  {key:'guangdong', cn:'广东', en:'Guangdong'},
  {key:'hainan', cn:'海南', en:'Hainan'},
  {key:'taiwan', cn:'台湾', en:'Taiwan'},
  {key:'hongkong', cn:'香港', en:'Hong Kong'},
  {key:'macau', cn:'澳门', en:'Macau'},
];
...
const PINYIN_HOSTS = '.py, .dict-py, .converse-py, .translate-reveal, .hw-context, .hw-py-text, ' +
  ...
  '#conversePy, #revealPy, #dictatePy, #sylOut, #tonePanel';
...
// Neither Zilla Slab nor Noto Serif has real glyphs for the precomposed
// double-diacritic pinyin vowels (tone-marked ü, and the plain single-mark
// vowels below). Substituting a different font for the *whole* letter (tried
// first, twice — a unicode-range @font-face, then an explicit
...
// how closely the size is matched. So instead, don't substitute the letter at
// all: split it into a plain base letter — which Zilla Slab renders
// natively, guaranteed identical to the rest of the word — plus a small
// standalone accent mark positioned above it with CSS.
// Plain ü (bare, no tone mark) turns out to need none of this: pixel-bbox
// probing confirmed Zilla Slab has a real, native 'ü'/'Ü' glyph built on the
// same stem as its own 'u'/'U' (identical horizontal ink extent), so it's not
// in this decomposition table at all — it just renders as ordinary text. Only
// the four precomposed ü+tone glyphs (ǖǘǚǜ and uppercase) are missing from
// Zilla Slab; those decompose to the native 'ü'/'Ü' base plus a tone mark, same
// as every other vowel, just anchored higher (.py-acc-uu) to clear the
// umlaut's own dots instead of colliding with them.
...
// Bases whose own glyph already carries diacritic dots (ü/Ü's umlaut) need
// the tone mark shifted further up (.py-acc-uu) so it clears those dots
// instead of overlapping them — plain vowel bases sit lower (.py-acc).
const ACCENT_HIGH_BASE = new Set(['ü','Ü']);
...
function wrapPinyinGlyphFixes(word){
  ...
  for(const ch of word){
    if(ACCENT_DECOMPOSE[ch]){
      ...
      const accClass = ACCENT_HIGH_BASE.has(pair[0]) ? 'py-acc py-acc-uu' : 'py-acc';
      out += '<span class="py-acc-wrap"><span class="py-visual" aria-hidden="true">' + pair[0] +
        '<span class="' + accClass + '">' + TONE_ACCENT_MARK[pair[1]] + '</span></span><span class="py-copy">' + ch + '</span></span>';
    } else {
      out += ch;
    }
  }
...
(function injectPinyinGlyphFixStyle(){
  ...
  style.textContent =
    ...
    ".py-visual{-webkit-user-select:none; user-select:none;}" +
    // top:0.38em — this is the ORIGINAL value, restored. Two later attempts
    // this session (-1.1em, then a calc(-0.78em - 3.5px) hybrid) tried to
    // fix it as a SITEWIDE problem, on the mistaken assumption that the
    // overlap reported in Dictate's "also reads as"/definitions list meant
    // 0.38em was wrong everywhere — but 0.38em was actually rendering
    // correctly on every other page (Writing Practice, Multiple Choice,
    // Search, Pinyin Components, Type Pinyin) the whole time, and both
    // "fixes" broke all of those working contexts to chase one page's own
    // problem, which is why the accent then read as floating too high
    // everywhere else. The real, narrower fix lives as a scoped override in
    // dictate.html itself (see its own <style> block) targeting just its
    // definitions panel — this shared base value is back to what every
    // other context on the site was already relying on.
    ...
    // Shifted up from the plain .py-acc's own top by the same 0.22em gap as
    // before (pixel-bbox probing showed ü's own umlaut dots sit that much
    // higher above baseline than a plain vowel's ink-top) — restored
    // alongside the base value above; see that comment for why.
    ".py-acc.py-acc-uu{top:0.16em;}" +
    ...
    // Selecting pinyin used to show the hidden TRUE-font letter instead of the
    // reworked one. .py-copy sits on top of the visual layer, so its selection
    // highlight (an opaque box) covered the reworked glyph, and the sitewide
    // ::selection text colour overrode its color:transparent, revealing the
    // real precomposed character (fallback font, own dot and all) in its
    // place. Fixed in three parts, all selection-only — nothing unselected
    // moves or changes:
    //  1. .py-copy stays selectable (so clicks/drags/copy behave exactly as
    //     before) but paints nothing while selected: no box, no ink.
    ".py-copy::selection{background:transparent; color:transparent; -webkit-text-fill-color:transparent;}" +
    //  2. The highlight box is drawn by the wrapper instead — same box as the
    //     letter's line, so it lines up with the neighbours' native highlight.
    //  3. The (unselectable) visual layer takes the selected-text colour, so
    //     the reworked glyph reads on the highlight like every other letter.
    // .py-selected is kept in sync with the real selection by
    // trackPinyinSelection below.
    ".py-acc-wrap.py-selected{background:var(--seal);}" +
    ".py-acc-wrap.py-selected .py-visual{color:var(--paper);}";
  document.head.appendChild(style);
})();
// Mirrors the current text selection onto the reworked accented letters (see
// .py-selected above). Cheap: nothing runs while the selection is collapsed
// and no letter is marked, and updates are batched to at most one per ~16ms
// (a timeout rather than requestAnimationFrame, which browsers pause for
// background tabs).
(function trackPinyinSelection(){
  let scheduled = false;
  let marked = [];
  function update(){
    scheduled = false;
    ...
    if(!active && !marked.length) return;
    const next = [];
    if(active){
      const wraps = document.querySelectorAll('.py-acc-wrap');
      for(let i = 0; i < wraps.length; i++){
        if(sel.containsNode(wraps[i], true)) next.push(wraps[i]);
      }
    }
    marked.forEach(function(w){ w.classList.remove('py-selected'); });
    next.forEach(function(w){ w.classList.add('py-selected'); });
    marked = next;
  }
...
// "Type anywhere" for the typing practice pages (Translate, Type Pinyin,
// Dictate): a printable key pressed while the answer box isn't focused —
// e.g. right after clicking Reveal/Shuffle/a mode pill, or before ever
// clicking the box — focuses it and the character lands there, instead of
// being dropped. getInput() returns the page's current answer <input> (or
// null when there isn't one: write mode, round summary, empty state).
// Skipped when: a field/editor already has focus, a modifier is held
// (shortcuts), the key isn't a single printable non-space character (Space and
// Enter keep working on a focused button/slider), a modal is open, or the box
// is missing/readonly/hidden (the idle preview's demo input is readonly).
function ccBindTypeAnywhere(getInput){
  document.addEventListener('keydown', function(e){
    ...
    if(!input || input.disabled || input.readOnly || input.offsetParent === null) return;
    input.focus();
    try{ const n = input.value.length; input.setSelectionRange(n, n); }catch(err){}
    // The box's own keydown handlers (the 1-5 tone-number keys) never saw this
    // event, since it was dispatched to whatever had focus before. Replay it
    // to the box so they run; if one handles it (preventDefault), suppress the
    // native insert too, otherwise the character types normally.
    const replay = new KeyboardEvent('keydown', { key: e.key, code: e.code, bubbles: true, cancelable: true });
    if(!input.dispatchEvent(replay)) e.preventDefault();
  }, true);
}
...
const FOCUS_CATEGORIES = [
  ...
  {key:'hearing', label:'Hearing', short:'A', varColor:'--slate'},
];
...
/* ---------- vocabulary list progress metric ('added' vs 'known') ----------
   Which of the two progress numbers a subscribed vocabulary list's card
   shows as its front-facing (bold, and what drives the % + progress bar)
   measure — reported directly ("make it so that it switches whenever the
   box gets clicked... both on here [word-bank.html] and on index.html").
   One shared preference, not per-card and not per-page: toggling any one
   card flips every card on both pages together, so this lives here (loaded
   on every page) rather than as a page-local variable. Defaults to 'added'
   — the metric this used to be the ONLY one, before Known was split out. */
function loadVocabListMetric(){
  try{ return localStorage.getItem('ccVocabListMetric') === 'known' ? 'known' : 'added'; }catch(e){ return 'added'; }
}
function saveVocabListMetric(metric){
  try{ localStorage.setItem('ccVocabListMetric', metric === 'known' ? 'known' : 'added'); }catch(e){}
}
function toggleVocabListMetric(){
  saveVocabListMetric(loadVocabListMetric() === 'added' ? 'known' : 'added');
}
...
function isEntryFocused(hz, category){
  const chars = Array.from(hz).filter(function(ch){ return HAN_CHAR_RE.test(ch); });
...
// Looser than isEntryFocused's own "every character" check above (which
// answers "does this whole Word Bank row deserve a lit badge") — this
// answers a different question, "does ANY part of this word need extra
// exposure", used by Read/Converse's own generation prompts (see
// gemini-generator.js) to decide which folder words to feature more often:
// tagging just the one hard character in a multi-character word should
// still boost the whole word, not require every character in it to be
// individually flagged first.
function anyCharFocused(hz, category){
  return Array.from(hz).some(function(ch){ return isFocusedIn(ch, category); });
}
// A page-agnostic version of word-bank.html's own onToggleWordFocusTag —
// same Known/skill-tag mutual-exclusion rule (turning a skill tag ON
// clears Known), just without that function's own final renderWordBankView()
// call, so pages other than Word Bank (Read's per-character Reading tag,
// Converse's per-word Hearing tag) can toggle a focus tag without needing
// Word Bank's own view loaded.
function toggleWordFocusTagShared(hz, category, charBased){
  const turningOn = charBased ? !isEntryFocused(hz, category) : !isFocusedIn(hz, category);
  if(turningOn && isFocusedIn(hz, KNOWN_TAG.key)) setFocusCategory(hz, KNOWN_TAG.key, false);
  if(charBased) toggleEntryFocus(hz, category); else toggleFocusCategory(hz, category);
}

/* ---------- one-click copy-to-clipboard (Search + Word Bank) ----------
   renderOracleHz's own oracle-bone/seal-script markup (extra per-character
   wrapper spans, decorative dot spans, etc.) makes a plain drag-select-then-
   Ctrl+C copy of an entry's hanzi unreliable, reported directly — this is
   a dedicated, always-visible button instead (the same idea as mdbg.net's
   own copy icon next to each dictionary entry), copying the plain text hz
   string directly rather than anything rendered on screen. ccCopyToClipboard
   briefly swaps the button's own icon to a checkmark to confirm the copy
   actually happened, then reverts it — silent success is easy to miss. */
function ccCopyHanziButtonHTML(hz, title){
  const escHz = String(hz).replace(/'/g, "\\'");
  return `<button class="cc-copy-btn" title="${title || 'Copy'}" onclick="ccCopyToClipboard('${escHz}', this)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg></button>`;
}
function ccCopyToClipboard(text, btnEl){
  function finish(ok){
    if(!btnEl) return;
    const original = btnEl.innerHTML;
    const originalTitle = btnEl.title;
    if(ok){
      btnEl.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>';
      btnEl.title = 'Copied!';
      setTimeout(function(){ btnEl.innerHTML = original; btnEl.title = originalTitle; }, 1200);
    }
  }
  ...
  // Fallback for a context without the async Clipboard API (e.g. serving
  // over plain HTTP rather than HTTPS/localhost) — a temporary offscreen
  // textarea plus the older execCommand still works broadly.
  try{
...
/* ---------- Hanzi Writing Practice knobs (read by writing-practice.html) ---------- */
...
// \p{Script=Han} (not a [一-鿿]-style range) so this also matches
// CJK Unified Ideographs Extension B and beyond — characters outside the
// Basic Multilingual Plane, like 𰻝 (the "biáng" of biángbiáng noodles),
// stored as a surrogate pair. A bare BMP range silently excluded those from
// every Han-detection check on the site (Focus toggling, Writing/Match
// Pinyin/Type Pinyin deck-building, hasHanzi(), etc.).
...
const I18N = {
  'nav.learn': '学习', 'nav.vocabulary': '词汇', 'nav.hanzi': '汉字',
  'nav.pinyin': '拼音', 'nav.zhuyin': '注音', 'nav.china': '中国+',
  // nav.geography labels both the Geography dropdown link itself and (via
  // nav.history) the History sub-header inside the merged "China +" nav item.
  'nav.geography': '地理', 'nav.history': '历史', 'nav.culture': '文化',
  'learn.lessons': '课程', 'learn.grammar': '语法',
  'learn.read': '阅读', 'learn.dictate': '听写', 'learn.multiplechoice': '选择题', 'learn.translate': '翻译',
  'vocab.search': '搜索', 'vocab.wordbank': '生词库',
  'hanzi.strokes': '笔画', 'hanzi.radicals': '部首', 'hanzi.writing': '写字练习', 'pinyin.components': '拼音成分', 'pinyin.match': '拼音匹配', 'pinyin.typing': '拼音打字',
  'zhuyin.chart': '注音表',
...
/* ---------- collapsible section cards ----------
   Word Bank's Vocabulary lists/Import/Folders cards and every Settings card
   (Account/Data/Appearance/Geography/Audio/Practice) share this one
   mechanism — click a card's header (a `.collapsible-toggle` element) to
   toggle its own `.collapsible` ancestor, independently of every other
   card, any combination open at once. Collapsing is a plain CSS child
   selector (.collapsible.collapsed > *:not(:first-child){display:none}),
   not a wrapper div around each card's content, so none of that markup or
   the IDs inside it had to move.
   Collapsed is the DEFAULT, baked directly into each card's class="...
   collapsible collapsed" in the HTML — not applied by this script — so a
   first-ever visit renders correctly collapsed with no JS/localStorage
   round-trip needed and no flash of expanded content first. What's tracked
   here is the opposite: which cards a user has chosen to EXPAND, so those
   specific ones stay open on their next visit instead of the whole page
   reverting to fully collapsed every time. */
function loadExpandedSections(){
  try{ const raw = localStorage.getItem('ccExpandedSections'); return raw ? JSON.parse(raw) : []; }catch(e){ return []; }
}
function saveExpandedSections(arr){
  try{ localStorage.setItem('ccExpandedSections', JSON.stringify(arr)); }catch(e){}
}
function toggleCollapsible(headerEl){
  ...
  if(!id) return;
  const expanded = new Set(loadExpandedSections());
  if(nowCollapsed) expanded.delete(id); else expanded.add(id);
  saveExpandedSections(Array.from(expanded));
}
// Collapsed: a click anywhere on the box (its own background/padding, not
// just the narrow header line) opens it — there's nothing else to click yet,
// since a collapsed box hides everything but its header (see
// .collapsible.collapsed > *:not(:first-child) in each page's own CSS).
// Expanded: only the header itself (or its caret) closes the box back —
// the body is now full of real, individually-useful controls (toggles,
// dropdowns, drag handles, buttons), so treating its own padding/whitespace
// as "click anywhere to close" risked a stray click collapsing the box out
// from under whatever you were doing inside it. Either way, wire this to
// the OUTER .collapsible element's onclick (passing `event, this`) instead
// of the header's own onclick="toggleCollapsible(this)" — using both at
// once would double-toggle on a header click, since the header's own
// handler firing doesn't stop the outer one from also firing as the event
// bubbles up to it.
function toggleCollapsibleBox(e, card){
  if(card.classList.contains('collapsed')){
    const interactive = e.target.closest('button, a, input, select, textarea, label, [onclick]');
    if(interactive && interactive !== card && !interactive.classList.contains('collapsible-toggle')) return;
    toggleCollapsible(card);
    return;
  }
  if(e.target.closest('.collapsible-toggle')) toggleCollapsible(card);
}
function applyCollapsedSections(){
  const expanded = new Set(loadExpandedSections());
  if(!expanded.size) return; // HTML already renders collapsed by default — nothing to re-expand
  document.querySelectorAll('.collapsible[data-collapse-id]').forEach(function(card){
    if(expanded.has(card.getAttribute('data-collapse-id'))) card.classList.remove('collapsed');
  });
}
document.addEventListener('DOMContentLoaded', applyCollapsedSections);

/* ---------- study-session resume ----------
   Writing Practice / Match Pinyin / Type Pinyin's Folders and Rapid Fire
   modes each record which words a session has already gotten through, keyed
   by page + mode (+ which folders, for Folders) — so starting that same
   deck again later can offer to pick up where you left off instead of
   silently starting over, or just as silently re-serving words already
   done. page is a short prefix ('hw'|'mp'|'pt'); mode is 'rapid'|'folders';
   selectedFolders only matters (and is sorted, so selection order doesn't
   fragment the key) when mode is 'folders'. */
function loadStudyProgress(){
  try{ const raw = localStorage.getItem('ccStudyProgress'); return raw ? JSON.parse(raw) : {}; }catch(e){ return {}; }
}
function saveStudyProgressAll(all){
  try{ localStorage.setItem('ccStudyProgress', JSON.stringify(all)); }catch(e){}
}
function studyProgressKey(page, mode, selectedFolders){
  return mode === 'folders' ? page + ':folders:' + selectedFolders.slice().sort().join(',') : page + ':' + mode;
}
function getStudyProgressDone(page, mode, selectedFolders){
  const all = loadStudyProgress();
  return new Set(all[studyProgressKey(page, mode, selectedFolders)] || []);
}
function addStudyProgressDone(page, mode, selectedFolders, wordId){
  const key = studyProgressKey(page, mode, selectedFolders);
  const all = loadStudyProgress();
  const done = new Set(all[key] || []);
  if(done.has(wordId)) return;
  done.add(wordId);
  all[key] = Array.from(done);
  saveStudyProgressAll(all);
}
function clearStudyProgress(page, mode, selectedFolders){
  const all = loadStudyProgress();
  delete all[studyProgressKey(page, mode, selectedFolders)];
  saveStudyProgressAll(all);
}

/* Same visual pattern as the "Add to Word Bank?" prompt shown after
   subscribing to a Vocabulary List on Word Bank (a .modal-backdrop >
   .modal-card, radio choices, one Confirm button) — that markup lives on
   word-bank.html itself, so this rebuilds the same shape here for
   pages that need it, each supplying its own .modal-* CSS (page-owned, like
   everything else on this site) to render it. opts: {title, message,
   onContinue, onRestart} — onContinue also fires if the modal is dismissed
   without an explicit choice (closing it is not a request to lose progress). */
let _resumePromptCounter = 0;
function showResumeStudyPrompt(opts){
  // A fast double/triple click on "Start studying selected" re-enters this
  // same code path once per click (nothing upstream debounces it), and each
  // call used to create its own independent backdrop — invisible ones would
  // silently stack up behind whichever one is on top. Bail out on a second
  // call while one's already open instead: the first modal's own choice
  // (continue/restart) is what should actually apply, not a chain of extra
  // ones queued up behind it.
  if(document.querySelector('.modal-backdrop')) return;
  // Radio inputs sharing a `name` are mutually exclusive across the WHOLE
  // document when they're not inside a <form> (as these aren't) — a second
  // modal instance's "continue" radio checking itself would silently
  // uncheck the first one's, since the browser treats every same-named
  // radio anywhere on the page as one group. Suffixing name (and the
  // button id, for the same reason) with a per-call counter keeps two
  // concurrent instances fully independent.
  const uid = 'resume' + (++_resumePromptCounter);
  ...
  backdrop.innerHTML = `
    <div class="modal-card">
      ...
      <div class="modal-field">
        <label><input type="radio" name="${uid}" value="continue" checked> Continue from where I left off</label>
        <label><input type="radio" name="${uid}" value="restart"> Start from the top</label>
      </div>
      <div class="modal-actions">
        <button class="btn small" id="${uid}ConfirmBtn">Confirm</button>
      </div>
    </div>`;
  // Elements outside the browser's native fullscreen element aren't
  // rendered at all while fullscreen is active — appending to document.body
  // (a sibling of whatever page-specific element actually went fullscreen,
  // e.g. Writing Practice's #hwWrap) meant this modal was silently built and
  // sitting in the DOM, invisible, until fullscreen was exited. Appending
  // into the real fullscreen element when there is one fixes that; outside
  // fullscreen document.fullscreenElement is just null and this is
  ...
  backdrop.querySelector('.modal-close').onclick = function(){ opts.onContinue(); close(); };
  backdrop.addEventListener('click', function(e){ if(e.target === backdrop){ opts.onContinue(); close(); } });
  backdrop.querySelector('#' + uid + 'ConfirmBtn').onclick = function(){
    const choice = backdrop.querySelector(`input[name="${uid}"]:checked`).value;
    if(choice === 'restart') opts.onRestart(); else opts.onContinue();
    close();
  };
}

/* ---------- parental controls ----------
   A best-effort content filter, not an exhaustive one: CC-CEDICT (see
   cedict.js) is a real, crowd-sourced dictionary that documents vulgar and
   sexual-slang vocabulary alongside everything else, sometimes tagging it
   explicitly ("(vulgar)", "(taboo curse)", "(coarse language)") and
   sometimes not. Patterns below combine those tags with a hand-picked list
   of unambiguous profanity/sexual-slang/sex-trade/gambling terms, each
   checked as its own word (or word-start, for terms like "gambl" meant to
   also catch "gambling"/"gambler") so an innocent word merely containing
   the letters somewhere in the middle — "class", "assume", "assassinate" —
   never trips it. Deliberately left out: plain clinical/anatomical terms
   (e.g. "penis") and factual drug names (e.g. "heroin") — these are
   ordinary vocabulary, not the crude/explicit register this filter targets.
   A definition string always keeps this filter checking clauses that HAVE
   nothing to do with what makes it a match — over-filtering a stray
   inoffensive synonym in a multi-sense entry is an acceptable trade-off for
   never under-filtering the offensive one. */
const PARENTAL_CONTROL_PATTERNS = [
  // CC-CEDICT's own content-warning tags
  /\bvulgar/i, /\bobscen/i, /\btaboo curse\b/i, /\bcoarse language\b/i, /\bhighly offensive\b/i,
  // profanity
  /\bfuck/i, /\bcunt\b/i, /\bbastard\b/i, /\bbitch\b/i, /\basshole\b/i, /\bass\b/i, /\bshit\b/i, /\bdamn\b/i, /\bdick\b/i, /\bschlong\b/i,
  // sexual acts/slang and the sex trade
  /\bmasturbat/i, /\bejaculat/i, /\borgasm\b/i, /\bfellatio\b/i, /\bcunnilingus\b/i,
  /\bslut\b/i, /\bwhore\b/i, /\bhooker\b/i, /\bbrothel\b/i, /\bprostitut/i, /\bpimp\b/i, /\bporn/i,
  /\bsexual intercourse\b/i, /\bsex toy\b/i, /\bsex worker\b/i, /\bto have sex\b/i,
  // gambling — not flagged as "vulgar" by CC-CEDICT itself, grouped in here
  // at the user's own request to treat it the same way as the above
  /\bgambl/i, /\bcasino\b/i, /\bwager\b/i, /\bbookmaker\b/i, /\bbetting\b/i,
];
function parentalTextIsFlagged(text){
  return PARENTAL_CONTROL_PATTERNS.some(re => re.test(text));
}
// CC-CEDICT joins a word's separate senses with "; " (see cedict.js's own
// header comment) — splitting on that boundary and dropping only the
// senses that trip a pattern is what lets a word keep its other, ordinary
// meanings instead of losing the whole entry over one flagged sense.
// Returns null (not '') once every sense is gone, so callers can tell
// "still has content" from "fully blocked" and exclude the entry entirely
// in the latter case, rather than displaying an empty definition.
function filterCedictDefinition(en){
  const kept = en.split('; ').filter(sense => !parentalTextIsFlagged(sense));
  return kept.length ? kept.join('; ') : null;
}
// The single point every CEDICT consumer should route a raw [hz, py, en]
// triplet through instead of reading e[2] directly — tightening or
// loosening the filter, or changing what "blocked" means, only ever needs
// to change here. A no-op (returns {hz, py, en} unchanged) unless the
// setting is on; returns null when the whole entry should be hidden.
function parentalFilterCedictEntry(hz, py, en){
  if(!loadSettings().parentalControlsEnabled) return {hz, py, en};
  const filtered = filterCedictDefinition(en);
  return filtered === null ? null : {hz, py, en: filtered};
}
